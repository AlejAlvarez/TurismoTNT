...
export default function RecuerdosAlojamientoScreen ({route}) {
  ...
  const fetchRecuerdos = async () => {
    let result = await alojamientosFavMeta.getRecuerdos(item);
    console.log(result);
    let arrayRecuerdos = [];
    let id = fotoId;
    result.forEach( src =>{
  ...
  const agregarNuevoRecuerdo = async source =>{
    await alojamientosFavMeta.agregarRecuerdo(item, source);
  }
  ...
  const eliminarRecuerdo = async source =>{
    await alojamientosFavMeta.eliminarRecuerdo(item, source);
...
// Botón circular para agregar, puede servir la onda para el filtro del mapa
